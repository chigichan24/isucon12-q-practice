...
type PlayerScoreAndPlayerRow struct {
	...
	PlayerRawID   string `db:"player.id"`
...
func competitionRankingHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
		"SELECT player_score.tenant_id AS \"player_score.tenant_id\", player_score.player_id AS \"player_score.player_id\", player_score.id AS \"player_score.id\", player_score.competition_id AS \"player_score.competition_id\", player_score.score AS \"player_score.score\", player_score.row_num AS \"player_score.row_num\", player_score.created_at AS \"player_score.created_at\", player_score.updated_at AS \"player_score.updated_at\", player.id AS \"player.id\", player.display_name AS \"player.display_name\" FROM player_score INNER JOIN player ON player_score.player_id = player.id WHERE player_score.competition_id = ? ORDER BY player_score.row_num DESC",
		competitionID,
	); err != nil {
